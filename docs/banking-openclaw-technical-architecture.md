...
- 轻量 `Execution Coordinator`：负责把当前会话中的用户输入、确认动作、打断操作和前端实时反馈接入到任务执行链路
...
- `Task Planner`：作为远程规划服务负责从 `Session` / `Task` 上下文中生成任务计划、子任务结构、依赖关系和交付结构
- `Task Graph Runtime`：作为远程长任务执行服务负责 DAG 调度、状态推进、等待外部事件、重试、补偿和超时处理
- `Task State Machine`：作为远程状态服务负责维护任务节点和任务实例的权威状态流转
- `Task Plan Store Service`：作为计划主服务负责 `TaskPlanVersion`、计划差异、确认记录和重规划版本的持久化，并将最终数据保存到远程服务器数据库
- `Task Aggregator`：作为远程汇总服务负责汇总中间结果并生成统一交付物、摘要、草稿和待办
- `Replan Manager`：作为远程重规划服务负责在新材料、失败、目标变化或外部事件进入时生成新的计划版本
...
- 调度任务节点，处理等待、重试、回退、补件、升级人工和外部回执
- 汇总中间结果，统一回写 `Session`、`Task` 和证据链
- 在执行条件变化时触发 `Replan`，并保留完整计划版本

### 协作边界

- 本地控制面负责“让用户看见并控制执行过程”，不负责保存任务计划和执行状态的最终业务真相
- 远程服务负责“生成计划、驱动执行、维护状态机并持久化执行结果”，不直接承接前端实时连接
- 任务计划、节点状态、人工确认记录、回退记录和 `Replan` 版本最终都必须通过远程服务写入远程服务器数据库
...
- 相当于把个人助手中的多步 tool loop 升级为企业级任务图和状态机
...
sequenceDiagram
    ...
    Planner-->>User: 展示计划与确认节点
    Planner->>Agent: 下发可执行计划
    Agent->>Tool: 查询 / 预填 / 流程动作
    Tool->>Audit: 记录鉴权与调用
    Agent->>TaskRt: 回写节点结果与状态
...
2. `Replan Manager` 根据当前节点状态生成新的计划版本
3. 系统展示新旧计划差异
4. 若风险级别上升，则要求重新确认
